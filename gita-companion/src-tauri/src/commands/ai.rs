...
fn handle_startup_line(app: &AppHandle, line: &str) {
    if line.contains("Loading router embeddings") {
        ...
        emit_status(
            app,
            "processing",
            "Loading base model (first run may download)...",
        );
...
#[tauri::command]
pub async fn init_process(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
        ...
        let line = {
            ...
            match tokio::time::timeout(Duration::from_secs(30), rx.recv()).await {
                ...
                Ok(None) => {
                    ...
                    return Err(format!(
                        "gita_ai exited during startup. Last stderr: {err_detail}"
                    ));
                }
                Err(_) => {
                    ...
                    return Err(format!(
                        "gita_ai stopped producing output. stderr: {err_detail}"
                    ));
                }
            }
        };
